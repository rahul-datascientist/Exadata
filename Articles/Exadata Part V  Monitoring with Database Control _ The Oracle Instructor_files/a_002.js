...
var WPCOMSharing = {
	...
	get_counts : function() {
		...
		for ( var url in WPCOM_sharing_counts ) {
			...
			if ( WPCOMSharing.done_urls[ postId ] ) {
				continue;
			}

			// get both the http and https version of these URLs
			var https_url = url.replace( /^http:\/\//i, 'https://' ),
				http_url  = url.replace( /^https:\/\//i, 'http://' );

			if ( jQuery( '#sharing-facebook-' + postId ).length ) {
				WPCOMSharing.bump_sharing_count_stat( 'facebook' );
				facebookPostIds.push( postId );
			}

			// twitter
			if ( jQuery( '#sharing-twitter-' + postId ).length ) {
				WPCOMSharing.bump_sharing_count_stat( 'twitter' );
			...
			if ( jQuery( '#sharing-linkedin-' + postId ).length ) {
				WPCOMSharing.bump_sharing_count_stat( 'linkedin' );
			...
			WPCOMSharing.done_urls[ postId ] = true;
		}
