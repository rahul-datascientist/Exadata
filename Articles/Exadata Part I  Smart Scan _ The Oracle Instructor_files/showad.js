(function() {
	...
	function AdManager(linkToGlobalPart) {
		 ...
		 * @param {String} slotId
		 */
		this.displayAd = this.globalPart['displayAd'] = function(slotId, givenWidth, givenHeight){
			...
			width = givenWidth || parseInt(adTagEl.style.width, 10);
			height = givenHeight || parseInt(adTagEl.style.height, 10);
		...
		this.generateAdServerUrl = function(baseUrl, opts){
			opts = opts || {};
			var useSSL,url,knownParams,allParams,baseParams,userParamName,userParamValue,userParams,lang, referrerUrl;

			useSSL = 'https:' === document.location.protocol;
			allParams = [];
			knownParams = ['section_id', 'tz', 'fl', 'click3rd', 'slot_id', 'ref', 'pos'];
			lang = this.getLang();
			...
			if ( userParams = me.globalPart['customParams'] ){
				piterate: for ( userParamName in userParams ){
					for (var i = 0, l = knownParams.length; i < l; i++){
						if ( knownParams[i] + '' === userParamName + '' ){
							continue piterate;
						}
					}
					if ( userParams.hasOwnProperty(userParamName) ){
						userParamValue = userParams[userParamName];
						if ( me.isArray(userParamValue) ){
							for (var j = 0; j < userParamValue.length; j++){
								allParams.push( userParamName + '=' + encodeURIComponent( userParamValue[j] ) );
							}
							continue piterate;
						}
						allParams.push( userParamName + '=' + encodeURIComponent( userParamValue ) );
					}
				}
			}
